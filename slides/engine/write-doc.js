...
const BLOCKS = [
  ...
  { type: 'bold',  text: '基本方針：「訴求軸 × ターゲット × スタイル」の3変数でスライド全体を制御する' },
  { type: 'normal',text: 'スライドのデザインは「何を訴えるか（訴求軸）」「誰に向けて話すか（ターゲット）」「どう見せるか（スタイル）」の3変数で決まるという考えのもと設計した。この3変数をbrief（入力ファイル）に持たせ、それをClaude APIが読み込んでテキスト設計に変換、さらにGemini APIが画像として具現化する2段階のAIパイプラインを構築した。' },
  { type: 'empty' },
  { type: 'bold',  text: 'スタイル設計の考え方：' },
  { type: 'normal',text: '各スタイル（manus / stripe / apple など13種）は、実在するブランドや企業の視覚言語を参照している。これにより「プレミアム感」「親しみやすさ」「データ重視」など、ターゲットの感情に合わせたデザインを言語で指示できる。生成AIへの指示を「色とレイアウトの細かい数値」ではなく「ブランド参照」で行うことで、再現性と品質のバランスをとった。' },
  { type: 'empty' },
  ...
  { type: 'normal',text: '　→ briefの訴求軸・ターゲット・スタイルを読み込み' },
  ...
  { type: 'bold',  text: '良い点：' },
  { type: 'bullet',text: 'システムとして完結している：brief 1ファイルを変えるだけで、訴求軸・ターゲット・スタイルの異なる複数バリエーションを生成できる。評価タスクの「再現性」要件を満たしている。' },
  ...
  { type: 'bullet',text: 'デモスライド（8・9番）の完結性：現状はライブデモへの橋渡し用の静止画。スライド単体でデモの価値を伝えるには、GIF埋め込みやスクリーンショット合成の仕組みが必要。' },
];
