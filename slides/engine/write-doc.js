...
const BLOCKS = [
  ...
  { type: 'bullet', text: 'Geminiの確率的ハルシネーション ── 画像生成AIである以上、テキスト設計（image-design.json）が正しくても、Geminiが描画時に稀に同じフレーズを重複させる等の出力崩れが確率的に発生する。本システムでは --indexオプションで1枚単位の即時再生成ができる仕組みでカバーしているが、完全排除にはpptxやSlidev等のテンプレートベース描画への移行が必要。AIによる自由なレイアウトの良さを取るか、テキスト描画の確実性を取るかのトレードオフ。' },
